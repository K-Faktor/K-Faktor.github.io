...
(function ()
{
	...
	function makeRow(data)
	{
		...
		name.setAttribute("aria-label", "Имя DVAR");
		...
		value.title = "Красный, зелёный, синий и альфа от 0 до 1";
		value.setAttribute("aria-label", "Значение: красный зелёный синий альфа");
		...
		hex.title = "Цвет в hex, например #9e81b6";
		hex.setAttribute("aria-label", "Цвет в hex");
	...
	function renderOutput()
	{
		...
		fill(bindBox, bind === null ? "" : bind,
			"Введите клавишу бинда и действие, чтобы получить эту строку.");
		renderPreview();
	}
	...
	function copyBox(box, label)
	{
		...
		{
			setHint("В поле «" + label + "» пока нечего копировать.", true);
			return;
		}
		...
		{
			setHint(count === 1 ? "Строка бинда скопирована." : "Скопировано строк: " + count + " («" + label + "»).");
		}, function ()
		{
	...
	function init()
	{
		...
		if (reset !== null)
		{
			reset.addEventListener("click", function ()
			{
				renderRows(DEFAULT_ROWS);
				renderOutput();
				setHint("Сброшено к DVAR'ам по умолчанию.");
			});
		}
